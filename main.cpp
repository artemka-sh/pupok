...
public:
    ...
    double calc(const double* x) const override {
        ...
        Vec3d D(0.0, 0.0, 1.0);
        for (int i = 0; i < 4; ++i) {
            Matx33d Mx_step = getQuad(L_quad, x[i], true) * getDrift(L_drift);
            D = Mx_step * D;
        }
        double w = 1000.0;
...
void onTrackbar(int pos, void*) {
    ...
    drawPhaseEllipse(phase, st.bx, st.ax, eps_x, Scalar(0, 0, 255), "Beta: " + to_string(st.bx) + "; " + to_string(st.ax), 30);
    drawPhaseEllipse(phase, st.by, st.ay, eps_y, Scalar(255, 0, 0), "Alpha: " + to_string(st.by) + "; " + to_string(st.ay), 60);
    drawPhaseEllipse(phase, st.dx, st.dpx, 0, Scalar(0, 255, 0), "D: " + to_string(st.dx) + "; " + to_string(st.dpx), 90);
...
int main() {
    ...
    for(size_t i=1; i<history.size(); ++i) {
        ...
        line(plot, toPx(history[i-1].s, history[i-1].dx), toPx(history[i].s, history[i].dx), Scalar(0,200,0), 2, LINE_AA);
    }

    line(plot, Point(50, h-50), Point(w-50, h-50), Scalar(0,0,0), 2);
    putText(plot, "Envelope: Red(X), Yel(Y). Green Line = Dispersion Dx", Point(60, 40), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(50,50,50), 1);
